...
function displayAvailableRooms(rooms) {
    ...
    availableRoomsContainer.innerHTML = rooms.map(room => {
        ...
        return `
            <div class="room-option" onclick="window.selectRoom(${room.id}, ${pricePerNight})" id="room-${room.id}">
    ...
    if (bookingData.roomId) {
        ...
        if (preSelectedRoom) {
            selectRoom(preSelectedRoom.id, parseFloat(preSelectedRoom.price_per_night));
        }
    }
}
...
async function handleBookingSubmit(event) {
    ...
    try {
        ...
        showBookingConfirmation(result.booking || result);
